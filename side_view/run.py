...
def detect_spine_keypoints_dynamically(sp_kpts: List, sp_scores: List, 
    ...
    if len(valid_points) < 3:
        return {}
    valid_points.sort(key=lambda p: p[2])  # y 기준
...
# ========= (8) render_display_frame 최적화 =========
def render_display_frame(ctx: Context, img_bgr: np.ndarray, result: Dict[str, Any]) -> np.ndarray:
    """
    Streamlit 출력용 프레임 렌더링
    - 최종 해상도: Config.STREAMLIT_OUTPUT_W x Config.STREAMLIT_OUTPUT_H (고정)
    - 단일 리사이즈로 처리 (중복 제거)
    """
    ...
    # 원본 이미지에 직접 그리기 (복사 필요시 여기서)
    canvas = img_bgr.copy()  # 원본 보존
    ...
    if has_valid:
        ...
        for i in ctx.config.NECK_IDX:
            if i < len(use["sp_kpts"]) and i < len(use["sp_scores"]):
                x, y = use["sp_kpts"][i]; sc = use["sp_scores"][i]
                if sc > 0.2:
        ...
        for i in ctx.config.NECK_IDX + ctx.config.LUMBAR_IDX:
            if i < len(use["sp_kpts"]) and i < len(use["sp_scores"]):
                kpt = use["sp_kpts"][i]; sc = use["sp_scores"][i]
                if sc > 0.2:
        ...
        if len(neck_pts) >= 2:
            ...
            for (x, y) in neck_pts:
                ...
                cv2.circle(canvas, (x, y), 7, neck_color, 2)
        ...
        if len(lumbar_pts) >= 2:
            ...
            for (x, y) in lumbar_pts:
                cv2.circle(canvas, (x, y), 5, (255, 255, 255), -1)
                cv2.circle(canvas, (x, y), 7, lumbar_color, 2)
    else:
    ...
    if canvas.shape[1] != target_w or canvas.shape[0] != target_h:
        ...
        scale = min(scale_w, scale_h)  # 비율 유지 (letterbox)
        
        # 리사이즈 (고품질)
        if scale < 1.0:
            interp = cv2.INTER_AREA  # 축소
        else:
            interp = cv2.INTER_LANCZOS4  # 확대
    ...
    return canvas  # 최종 크기: STREAMLIT_OUTPUT_W x STREAMLIT_OUTPUT_H
        ...
        if now_ms >= ctx.next_bp_ts:
            ...
            if 0 < ctx.infer_scale < 1.0:
                ...
                if res and res.pose_landmarks:
                    ...
                    scale_inv = 1.0 / ctx.infer_scale  # 미리 계산
                    lm_px = {
                        k: (int(v[0] * scale_inv + 0.5),  # round 대신
...
def display_worker(ctx: Context):
    ...
    while ctx.running:
        ...
        # 이미 Streamlit 해상도로 도착
        # 로컬 윈도우 크기와 다르면 다시 리사이즈
...
def create_context(spine_est: SpinePoseEstimator, mp_pose, pose, 
    ...
    return Context(
        ...
        pose=pose,
        config=config,
        spine_only=True,  # 항상 spine_only 모드
        infer_scale=config.INFER_SCALE
    )

# ========= (13) main 함수에 해상도 설정 추가 =========
def main():
    """메인 실행 함수"""
    try:
        cv2.setNumThreads(1)
    ...
    parser = argparse.ArgumentParser(description="SpinePose Analysis (Fixed Streamlit Resolution)")
    ...
    print(f"Streamlit output: {args.streamlit_width}x{args.streamlit_height}")  # ✅ 추가
